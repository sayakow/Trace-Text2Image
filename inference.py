        ...
        with torch.no_grad():
            ...
            for l, (low, high) in enumerate(config.boundaries, 1):
                ...
                for step in range(1, total_steps):
                    ...
                    #特定の単語を埋め込む場合はここをコメントアウト①(⑤まで)
                    #special_word = ["dining table", "spoon","sandwich","knife","fork","broccoli","sandwich","broccoli"]
                    special_word = ["people"]

                    #特定の単語を埋め込む場合はここをコメントアウト②
                    #Step1の時に好きな単語入れる
                    if(step == 1):
                        for origin in range(len(pred_token_ids[0])):

                            #特定の単語を埋め込む場合はここをコメントアウト③（どれか選ぶ）

                            #文の真ん中に1回
                            if(origin == int(len(pred_token_ids.view(-1)[mask_id])*0.5)):
                                print("文長",int(len(pred_token_ids[0])))
                                pred_token_ids[0][origin] = tokenizer.encode(special_word[0])[0]



                    #print("[MASK]掛け直し",tokenizer.decode(pred_token_ids[0].cpu().numpy()))
                    #特定の単語を埋め込む場合はここをコメントアウト④
                    #全て[MASK]か.になった場合
                    mask_num = 0
                    must_word = 0
                    for origin in range(len(pred_token_ids[0])):
                        #[MASK]か.をカウント
                        if((pred_token_ids[0][origin] == 1012)or(pred_token_ids[0][origin] == 103)or(pred_token_ids[0][origin] == 1037)):
                            mask_num = mask_num + 1
                        #特定の単語をカウント
                        #if((pred_token_ids[0][origin] in tokenizer.encode(special_word)[0])):
                        if(special_word[0] in tokenizer.decode(pred_token_ids[0].cpu().numpy())):
                            must_word = must_word + 1
                    if(mask_num == len(pred_token_ids[0])):
                        for origin in range(len(pred_token_ids[0])):
                            #上で決めたif文をそのままh貼り付ける
                            if(origin == int(len(pred_token_ids.view(-1)[mask_id])*0.5)):
                                print("文長",int(len(pred_token_ids[0])))
                                print("[MASK]/...：前",tokenizer.decode(pred_token_ids[0].cpu().numpy()))
                                pred_token_ids[0][origin] = tokenizer.encode(special_word[0])[0]
                                print("[MASK]/...：後",tokenizer.decode(pred_token_ids[0].cpu().numpy()))
                    #特定の単語を埋め込む場合はここをコメントアウト⑤
                    # 必ず入れたい言葉が消えた場合
                    if( must_word == 0 ):
                    #elif(must_word < int(len(special_word)/3)):
                        for origin in range(len(pred_token_ids[0])):
                            #上で決めたif文をそのままh貼り付ける
                            if(origin == int(len(pred_token_ids.view(-1)[mask_id])*0.5)):
                                print("文長",int(len(pred_token_ids[0])))
                                print("単語再追加：前",tokenizer.decode(pred_token_ids[0].cpu().numpy()))
                                pred_token_ids[0][origin] = tokenizer.encode(special_word[0])[0]
                                print("単語再追加：後",tokenizer.decode(pred_token_ids[0].cpu().numpy()))
